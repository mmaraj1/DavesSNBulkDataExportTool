...
class SNDataExport:
    ...
    def run(self):
        ...
        try:
            ...
            #     #w = unicodecsv.UnicodeWriter('test.csv', encoding='utf-8')
            #     #w.writerow(row)
    ...
    def makeRequest(self, table, params):
        ...
        headers = {self.HEADER_CONTENTTYPE: self.HEADER_CONTENTTYPE_DEFAULT,
                   self.HEADER_ACCEPT: self.HEADER_ACCEPT_DEFAULT}
        auth = False
...
parser = argparse.ArgumentParser(
    description="David McDonald's ServiceNow Bulk Data Export tool 2021.\nFor exporting bulk amounts of data out of ServiceNow.")
parser.add_argument("-v", "--verbose", action="store_true", help="Enable verbose logging.")
parser.add_argument("-p", "--pagesize", dest="pageSize", type=int, default=SNDataExport.PAGESIZE_DEFAULT,
                    help="Rows per page / request. Default: {0}".format(SNDataExport.PAGESIZE_DEFAULT))
parser.add_argument("-t", "--table", type=str, required=True,
                    help="The ServiceNow table name to export from. E.g. 'incident'")
parser.add_argument("-n", "--instance-name", dest="instanceName", type=str,
                    help="The name of the ServiceNow instance. E.g. 'dev71826'")
parser.add_argument("-N", "--instance-url", dest="instanceUrl", type=str,
                    help="The hostname URL of the ServiceNow instance, if the instance has a custom URL. E.g. 'dev71826.custom-url.co.uk")
parser.add_argument("-d", "--display-value", action="store_true", help="Fetch displayvalues, instead of system values")
parser.add_argument("-o", "--output", dest="outputName", type=str, required=True,
                    help="Name of the file to save the data to.")
parser.add_argument("-a", "--auth-mode", dest="authType", choices=["none", "basic"], default="none",
                    help="Type of authentication. Default: none")
parser.add_argument("-q", "--query", type=str, default="ORDERBYsys_id",
                    help="Query to use on the table when fetching data.")
...
parser.add_argument("-f", "--fields", type=str,
                    help="A comma-separated list of fields to include. Can dot-walk through reference fields (e.g. caller_id.email). Leave blank for all fields.")
